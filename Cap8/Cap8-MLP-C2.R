...
setwd("~/PGMC - 2017-2021/Tese - parte 2/Códigos/MLP - nlcor")

nlcors <- read.csv('nlcors.csv')
nlcors <- nlcors[,2:12]
rownames(nlcors) <- zone_names[zones_wanted]
colnames(nlcors) <- 1:11

for(z in zones_wanted[6]) {
  ...
  setwd(paste("~/PGMC - 2017-2021/Tese - parte 2/Códigos/MLP - Hong - nlcor/",zone_names[z],sep=''))
  ...
  for(t in 1:11) {
    ...
    lr_final_t <- as.numeric(lr_final[as.numeric(rownames(stats_ordered[1,])),2]) * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    bs_final_t <- as.numeric(bs_final[as.numeric(rownames(stats_ordered[1,])),2]) * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    ep_final_t <- as.numeric(ep_final[as.numeric(rownames(stats_ordered[1,])),2]) * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    hu_final_t <- as.numeric(hu_final[as.numeric(rownames(stats_ordered[1,])),2]) * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    
    predicts_train[[t]] <- vector(mode = "list", 100)  
    predicts[[t]]       <- vector(mode = "list", 100)
    
    temp <- data_temps[,stats_ordered$t[1]] * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    
    nlcorsum <- nlcors[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    
    if(t > 1) {
      
      for(k in 2:t) {
        
        temp <- temp + data_temps[,stats_ordered$t[k]] * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        
        lr_final_t <- lr_final_t + as.numeric(lr_final[as.numeric(rownames(stats_ordered[k,])),2]) * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        bs_final_t <- bs_final_t + as.numeric(bs_final[as.numeric(rownames(stats_ordered[k,])),2]) * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        ep_final_t <- ep_final_t + as.numeric(ep_final[as.numeric(rownames(stats_ordered[k,])),2]) * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        hu_final_t <- hu_final_t + as.numeric(hu_final[as.numeric(rownames(stats_ordered[k,])),2]) * nlcors[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        
        nlcorsum <- nlcorsum + nlcors[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        
      }
    }
    
    temp <- temp/nlcorsum
    lr_final_t <- lr_final_t/nlcorsum
    bs_final_t <- round(bs_final_t/nlcorsum)
    ep_final_t <- round(ep_final_t/nlcorsum)
    hu_final_t <- round(hu_final_t/nlcorsum)
    ...
    write.csv(mape,file=paste('ffnn_hong_nlcor_mape_t_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r1,file=paste('ffnn_hong_nlcor_mape_r1_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r2,file=paste('ffnn_hong_nlcor_mape_r2_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r3,file=paste('ffnn_hong_nlcor_mape_r3_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r4,file=paste('ffnn_hong_nlcor_mape_r4_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r5,file=paste('ffnn_hong_nlcor_mape_r5_',zone_names[z],'_',t,'.csv',sep=''))
  ...
  for(t in 1:11) {
    ...
    write.csv(ffnn_predicts_train,file=paste('ffnn_predicts_hong_nlcor_train_',zone_names[z],'_',temp_names[t],'.csv',sep=''))
    
    write.csv(ffnn_predicts,file=paste('ffnn_predicts_hong_média_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(ffnn_predicts_r1,file=paste('ffnn_predicts_hong_nlcor_',zone_names[z],'_',t,'_r1.csv',sep=''))
    write.csv(ffnn_predicts_r2,file=paste('ffnn_predicts_hong_nlcor_',zone_names[z],'_',t,'_r2.csv',sep=''))
    write.csv(ffnn_predicts_r3,file=paste('ffnn_predicts_hong_nlcor_',zone_names[z],'_',t,'_r3.csv',sep=''))
    write.csv(ffnn_predicts_r4,file=paste('ffnn_predicts_hong_nlcor_',zone_names[z],'_',t,'_r4.csv',sep=''))
    write.csv(ffnn_predicts_r5,file=paste('ffnn_predicts_hong_nlcor_',zone_names[z],'_',t,'_r5.csv',sep=''))
  }
}