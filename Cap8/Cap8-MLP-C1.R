...
zones_wanted <- c(11,4,2,9,7,1,5)

setwd("~/PGMC - 2017-2021/Tese - parte 2/Códigos/MLP - Temperaturas em separado - centróide")

dist <- read.csv('distances.csv')
dist <- dist[,2:12]
dist[2,] <- (dist[2,]+dist[3,])/2
dist <- dist[-3,]

rownames(dist) <- zone_names[zones_wanted]
colnames(dist) <- 1:11

invdist <- 1/dist

for(z in zones_wanted[4:7]) {
  
  setwd(paste("~/PGMC - 2017-2021/Tese - parte 2/Códigos/MLP - Temperaturas em separado - resultados/",sep=''))
  
  stats <- read.csv(paste('stats_',zone_names[z],'.csv',sep=''))
  stats_ordered <- stats[order(stats$Mediana),]
  ...
  setwd(paste("~/PGMC - 2017-2021/Tese - parte 2/Códigos/MLP - Temperaturas em separado - centróide/",zone_names[z],sep=''))
  ...
  for(t in 1:11) {
    ...
    lr_final_t <- as.numeric(lr_final[as.numeric(rownames(stats_ordered[1,])),2]) * invdist[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    bs_final_t <- as.numeric(bs_final[as.numeric(rownames(stats_ordered[1,])),2]) * invdist[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    ep_final_t <- as.numeric(ep_final[as.numeric(rownames(stats_ordered[1,])),2]) * invdist[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    hu_final_t <- as.numeric(hu_final[as.numeric(rownames(stats_ordered[1,])),2]) * invdist[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    
    predicts_train[[t]] <- vector(mode = "list", 100)  
    predicts[[t]]       <- vector(mode = "list", 100)
    
    temp <- data_temps[,stats_ordered$t[1]] * invdist[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    
    invdistsum <- invdist[zone_names[z],as.numeric(rownames(stats_ordered[1,]))]
    
    if(t > 1) {
      
      for(k in 2:t) {
        
        temp <- temp + data_temps[,stats_ordered$t[k]] * invdist[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        
        lr_final_t <- lr_final_t + as.numeric(lr_final[as.numeric(rownames(stats_ordered[k,])),2]) * invdist[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        bs_final_t <- bs_final_t + as.numeric(bs_final[as.numeric(rownames(stats_ordered[k,])),2]) * invdist[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        ep_final_t <- ep_final_t + as.numeric(ep_final[as.numeric(rownames(stats_ordered[k,])),2]) * invdist[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        hu_final_t <- hu_final_t + as.numeric(hu_final[as.numeric(rownames(stats_ordered[k,])),2]) * invdist[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        
        invdistsum <- invdistsum + invdist[zone_names[z],as.numeric(rownames(stats_ordered[k,]))]
        
      }
    }
    
    temp <- temp/invdistsum
    lr_final_t <- lr_final_t/invdistsum
    bs_final_t <- round(bs_final_t/invdistsum)
    ep_final_t <- round(ep_final_t/invdistsum)
    hu_final_t <- round(hu_final_t/invdistsum)
    ...
    write.csv(mape,file=paste('ffnn_hong_centroid_mape_t_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r1,file=paste('ffnn_hong_centroid_mape_r1_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r2,file=paste('ffnn_hong_centroid_mape_r2_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r3,file=paste('ffnn_hong_centroid_mape_r3_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r4,file=paste('ffnn_hong_centroid_mape_r4_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(mape_r5,file=paste('ffnn_hong_centroid_mape_r5_',zone_names[z],'_',t,'.csv',sep=''))
  ...
  for(t in 1:11) {
    ...
    write.csv(ffnn_predicts_train,file=paste('ffnn_predicts_hong_centroid_train_',zone_names[z],'_',temp_names[t],'.csv',sep=''))
    
    write.csv(ffnn_predicts,file=paste('ffnn_predicts_hong_centroid_',zone_names[z],'_',t,'.csv',sep=''))
    write.csv(ffnn_predicts_r1,file=paste('ffnn_predicts_hong_centroid_',zone_names[z],'_',t,'_r1.csv',sep=''))
    write.csv(ffnn_predicts_r2,file=paste('ffnn_predicts_hong_centroid_',zone_names[z],'_',t,'_r2.csv',sep=''))
    write.csv(ffnn_predicts_r3,file=paste('ffnn_predicts_hong_centroid_',zone_names[z],'_',t,'_r3.csv',sep=''))
    write.csv(ffnn_predicts_r4,file=paste('ffnn_predicts_hong_centroid_',zone_names[z],'_',t,'_r4.csv',sep=''))
    write.csv(ffnn_predicts_r5,file=paste('ffnn_predicts_hong_centroid_',zone_names[z],'_',t,'_r5.csv',sep=''))
  }
}